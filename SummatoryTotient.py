class TotientSum:
 ...
 '''
   ...
   if not totcache is None:
     ...
     for i in xrange(2,self.cache_size):
      ...
      totSum[i] = (f(i)*totSum[i] + totSum[i-1]) % mod
     self.sCache = totSum
 ...
 def Sum(self,n,k):
   ...
   while q >= 1:
    r = n % div
    div_next = div + r/q + 1
    s = (s - self.fSum(div,div_next) * self.Sum(n/div,k*div) ) % mod
    div = div_next
    q = n/div_next
   if k < self.maxK:
    self.kCache[k] = s
 ...
 def weighted_sum(self,w,start=2):
  ...
  for i in xrange(start,self.maxK+1):
    s = ( s + w(self.cap/i) * (F(i)-F(i-1))) % self.mod
  
  for k in xrange(1,self.maxK):
    s = (s + w(k) * (F(n/k) - F(n/(k+1))) % self.mod
  return s
